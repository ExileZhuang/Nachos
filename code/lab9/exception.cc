...
ExceptionHandler(ExceptionType which)
{
    ...
    if (which == SyscallException) {
        switch(type){
          case SC_Yield:{
             printf("This is SC_Yield, CurrentThreadId: %d,CurrentThreadName: %s\n",(currentThread->space)->getSpaceId(),currentThread->getName());
             currentThread->Yield();
             AdvancePC();
             break;
          }
...
//             printf("%s\n",filename); 
          ...
          case SC_Exec:{
	     printf("This is SC_Exec, CurrentThreadId: %d,CurrentThreadName: %s\n",(currentThread->space)->getSpaceId(),currentThread->getName());
	     ...
	     break;
          ...
          case SC_Join:{
             printf("This is SC_Join, CurrentThreadId: %d,CurrentThreadName: %s\n",(currentThread->space)->getSpaceId(),currentThread->getName());
             int spaceId = machine->ReadRegister(4);
             printf("Join_1\n");    
             printf("spaceId: %d\n",spaceId);          
             currentThread->Join(spaceId);
             printf("Join_2\n");
             //waitProcessEcitCode-返回Joinee的退出码
...
//             printf("%s\n",filename);
          ...
          case SC_Exit:{
	     printf("This is SC_Exit, CurrentThreadId: %d,CurrnetThreadName: %s\n",(currentThread->space)->getSpaceId(),currentThread->getName());
